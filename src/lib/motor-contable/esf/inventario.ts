...
export function hojaINVENTARIO(wb: ExcelJS.Workbook, r: ResultadoMotor, reg?: RegistroCeldas) {
  ...
  const detalleBaseRN = ((ultRN as any)?.activoCorriente.inventarioDetalle ?? []).map((x: any) => ({ codigo: String(x.codigo), nombre: x.nombre, valor: x.valor }))
  const fuentePrefijoRN = (pref: string) => detalleBaseRN.filter((x: any) => String(x.codigo).startsWith(pref))
  const rnNota = aplicarReglasNota('INVENTARIO', detalleBaseRN, (ultRN as any)?.activoCorriente.inventarioTotal ?? 0, reglasNota, fuentePrefijoRN)
